...
if($payment_type_level === 0) { // Level 0 payments	
	...
	} else if(in_array($params['payment_type'],array('PAYPAL','PRZELEWY24', 'POSTFINANCE', 'POSTFINANCE_CARD')) && $params['status'] == 100 && $params['tid_status'] == 100) {
...
elseif($payment_type_level == 2) { // Credit entry payment and Collections available            
	...
	if(in_array($params['payment_type'], array('INVOICE_CREDIT', 'CASHPAYMENT_CREDIT', 'MULTIBANCO_CREDIT'))) {
...
class NovalnetVendorScript {
    ...
    protected   $initial_payments = array('CREDITCARD', 'INVOICE_START', 'DIRECT_DEBIT_SEPA', 'GUARANTEED_INVOICE', 'PAYPAL', 'GUARANTEED_DIRECT_DEBIT_SEPA', 'ONLINE_TRANSFER', 'IDEAL', 'EPS', 'GIROPAY', 'PRZELEWY24', 'POSTFINANCE', 'CASHPAYMENT', 'POSTFINANCE_CARD', 'BANCONTACT', 'MULTIBANCO');
    
    /** Chargeback, bookback, return debit and refund payments - Level : 1 */
    protected   $chargeback_payments =  array('RETURN_DEBIT_SEPA', 'REVERSAL', 'CREDITCARD_BOOKBACK', 'CREDITCARD_CHARGEBACK', 'REFUND_BY_BANK_TRANSFER_EU', 'PAYPAL_BOOKBACK', 'PRZELEWY24_REFUND', 'CASHPAYMENT_REFUND', 'GUARANTEED_INVOICE_BOOKBACK', 'GUARANTEED_SEPA_BOOKBACK', 'POSTFINANCE_REFUND');
    
    /** Credit entry payment and collections payments - Level : 2 */
    protected   $collection_credit_payments = array('INVOICE_CREDIT', 'CREDIT_ENTRY_CREDITCARD', 'CREDIT_ENTRY_SEPA', 'DEBT_COLLECTION_SEPA', 'DEBT_COLLECTION_CREDITCARD', 'ONLINE_TRANSFER_CREDIT', 'CASHPAYMENT_CREDIT', 'CREDIT_ENTRY_DE', 'DEBT_COLLECTION_DE', 'MULTIBANCO_CREDIT');
    ...
    protected $payment_group = array(
        ...
        'novalnetpostfinance'   	=> array('POSTFINANCE', 'POSTFINANCE_REFUND'),
        'novalnetfinancecard'   	=> array('POSTFINANCE_CARD', 'POSTFINANCE_REFUND'),
        'novalnetbancontact'   		=> array('BANCONTACT', 'REFUND_BY_BANK_TRANSFER_EU'),
        'novalnetmultibanco'		=> array('MULTIBANCO', 'MULTIBANCO_CREDIT', 'REFUND_BY_BANK_TRANSFER_EU'),
   );   
        ...
        if($row = $db->db_fetch()) {
            ...
            if($order_id != '') { 
				...
				if ($this->capture_params['status'] == '100') {
					...
					else if(in_array($this->capture_params['tid_status'], array('75', '86', '90')) || in_array($row->payment_name, array('novalnetprepayment','novalnetcashpayment','novalnetmultibanco')))
