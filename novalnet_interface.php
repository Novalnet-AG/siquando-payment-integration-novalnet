// Novalnet code STARTS
			...
			case 'novalnetsepa':
			case 'novalnetcc':
				if(in_array($myorder->payment_param, array('novalnetprepayment','novalnetinvoice','novalnetsepa','novalnetcashpayment','novalnetmultibanco')) || ($myorder->payment_param == 'novalnetcc' && $pm->parameter[5] != '1' && $pm->parameter[6] != '1')) {
					...
					if(get('result') != 'success') {
						...
						if($payment_name == 'novalnetsepa') {
							...
							if (empty($novalnet_parameters['iban']))
								redirect(CC_URL_PAYMENT.'?'.PARAMETER_ERROR.('&nnmessage='.CC_SITE_NNSELECT_PAYMENT_TEXT));
						}
				...
				$myorder->db_updateobject();
							...
							} else if($payment_name == 'novalnetmultibanco') { // For Multibanco
								$bank_store_details .= get_multibanco_payment_reference($response['partner_payment_reference'], $response['service_supplier_id']);
								$template->show('__novalnet_multibancotrans');
								$template->assign(array('__novalnet_multibanco_transdesc' 	=>  CC_SITE_NNMULTIBANCOREFDESC));
								$template->assign(array('__novalnet_multibanco_transref'	=>  $response['partner_payment_reference']));
								$template->assign(array('__novalnet_multibanco_entityno' 	=>  $response['service_supplier_id']));
								$template->assign(array('__novalnet_multibanco_transamount' =>   format::price($myorder->get_totalprice(), $myorder->currency)));	
							...
							else if(in_array($response['tid_status'], array(75,86,90)) || in_array($payment_name, array('novalnetprepayment','novalnetcashpayment','novalnetmultibanco')))
